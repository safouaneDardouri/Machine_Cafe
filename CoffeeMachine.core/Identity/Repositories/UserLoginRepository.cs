...
using CoffeeMachine.Core.Identity;
    ...
    public class UserLoginRepository
    {
        ...
        public void Insert(IdentityUser user, UserLoginInfo login)
        {
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                CoffeeMachine.Core.Identity.MySqlHelper.ExecuteNonQuery(conn, @"INSERT INTO aspnetuserlogins(UserId,LoginProvider,ProviderKey) VALUES(@Id,@LoginProvider,@ProviderKey)", parameters);
            }
        }
        ...
        public void Delete(IdentityUser user, UserLoginInfo login)
        {
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                CoffeeMachine.Core.Identity.MySqlHelper.ExecuteNonQuery(conn, @"DELETE FROM aspnetuserlogins WHERE UserId = @Id AND LoginProvider = @LoginProvider AND ProviderKey = @ProviderKey", parameters);
            }
        }
        ...
        public string GetByUserLoginInfo(UserLoginInfo login)
        {
            ...
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                var userIdObject = CoffeeMachine.Core.Identity.MySqlHelper.ExecuteScalar(conn, CommandType.Text,
        ...
        public List<UserLoginInfo> PopulateLogins(string userId)
        {
            ...
            using (var conn = new MySqlConnection(_connectionString))
            {
                ...
                var reader = CoffeeMachine.Core.Identity.MySqlHelper.ExecuteReader(conn, CommandType.Text,
